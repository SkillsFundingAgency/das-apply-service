...
{
    public class When_valid_answers_on_multiple_answers_page_with_existing_answers_are_provided : UpdatePageAnswersHandlerTestBase
    {
        ...
        [Test]
        public void Then_section_is_saved_with_the_new_answers()
        {
            ...
            ApplyRepository.Verify(r => r.SaveSection(It.Is<ApplicationSection>(
                section => 
                    section.QnAData.Pages.First(p => p.PageId == "3").PageOfAnswers[1].Answers[0].QuestionId == AnswerQ4.QuestionId && 
                    section.QnAData.Pages.First(p => p.PageId == "3").PageOfAnswers[1].Answers[0].Value == AnswerQ4.Value), UserId), Times.Once());

            ApplyRepository.Verify(r => r.SaveSection(It.Is<ApplicationSection>(
                section =>
                    section.QnAData.Pages.First(p => p.PageId == "3").PageOfAnswers[1].Answers[1].QuestionId == AnswerQ5.QuestionId &&
                    section.QnAData.Pages.First(p => p.PageId == "3").PageOfAnswers[1].Answers[1].Value == AnswerQ5.Value), UserId), Times.Once());
        }
        ...
        public void Then_validation_is_not_carried_out_on_existing_answers()
        {
            ...
            Validator.Verify(v => v.Validate(_existingAnswerQ4), Times.Never());
            Validator.Verify(v => v.Validate(_existingAnswerQ5), Times.Never());
        }
    }
}