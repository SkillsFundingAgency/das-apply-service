...
{
    public class When_file_upload_answer_removed_on_single_file_page : DeleteFileHandlerTestBase
    {
        ...
        {
            Handler.Handle(new DeleteFileRequest(ApplicationId, UserId, "1", 1, 1, "Q1", "MyFile.pdf"),
                new CancellationToken()).Wait();

            ApplyRepository.Verify(r => r.SaveSection(It.Is<ApplicationSection>(
                section =>
                    section.QnAData.Pages.First(p => p.PageId == "1").PageOfAnswers[0].Answers.Count == 0),
                    UserId));
        }
        ...
        public void Then_section_is_saved_with_page_not_complete()
        {
            ...
            ApplyRepository.Verify(r => r.SaveSection(It.Is<ApplicationSection>(
                section => 
                    section.QnAData.Pages.First(p => p.PageId == "1").Complete == false), 
                    UserId));
