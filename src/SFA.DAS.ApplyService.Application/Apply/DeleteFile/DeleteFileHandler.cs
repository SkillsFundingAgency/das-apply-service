...
namespace SFA.DAS.ApplyService.Application.Apply.DeleteFile
{
    public class DeleteFileHandler : IRequestHandler<DeleteFileRequest>
    {
        ...
        public async Task<Unit> Handle(DeleteFileRequest request, CancellationToken cancellationToken)
        {
            ...
            if ((question.Input.Validations.Any(v => v.Name == "Required") && updatedPageOfAnswers.Answers.Count == 0) || 
                (question.Input.FileUploadInfo.NumberOfUploadsRequired != null && question.Input.FileUploadInfo.NumberOfUploadsRequired.Value > updatedPageOfAnswers.Answers.Count))
            {
                page.Complete = false;
            }
