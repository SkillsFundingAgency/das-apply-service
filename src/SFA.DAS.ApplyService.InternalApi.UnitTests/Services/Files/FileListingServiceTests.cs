...
namespace SFA.DAS.ApplyService.InternalApi.UnitTests.Services.Files
{
    public class FileListingServiceTests
    {
        ...
        private FileListingService _fileListingService;
        ...
        public async Task Setup()
        {
            ...
            _fileListingService = new FileListingService(_configurationService.Object);
        }
        ...
        {
            var result = await _fileListingService.GetFileList(_applicationId, _sequenceNumber, _sectionNumber, _pageId, _containerType, new CancellationToken());

            Assert.IsNotNull(result);
            CollectionAssert.Contains(result, _nameOfFileThatExists);
            CollectionAssert.Contains(result, _nameOfFileThatExists2);
        }
        ...
        [Ignore("Must be tested on local DEV machine as it uses local Azure storage")]
#endif
        public async Task GetFileList_when_page_directory_does_not_exist_Then_it_returns_null()
        {
            var nameOfPageThatDoesNotExist = $"{Guid.NewGuid()}";
            var result = await _fileListingService.GetFileList(_applicationId, _sequenceNumber, _sectionNumber, nameOfPageThatDoesNotExist, _containerType, new CancellationToken());
