...
namespace SFA.DAS.ApplyService.Web.StartupExtensions
{
    public class CustomClaims : ICustomClaims
    {
        ...
        public Task<IEnumerable<Claim>> GetClaims(ClaimsPrincipal claimsPrincipal)
        {
            var email = claimsPrincipal?.Claims.FirstOrDefault(c => c.Type.Equals(ClaimTypes.Email))?.Value;
            var claims = new List<Claim>
            {
                new ("Email",$"{email}"),
            };

            var sub = claimsPrincipal?.Claims.FirstOrDefault(c => c.Type == "sub")?.Value
